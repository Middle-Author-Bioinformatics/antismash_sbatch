...
def _kcb_list_significant_products(txt_text: str) -> list[str]:
    """
    Return a list of product names from the 'Significant hits:' section
    of a knownclusterblast .txt file.
    """
    products = []
    if not txt_text:
        return products

    # Match from "Significant hits:" to either "Details:" or EOF
    print(txt_text)
    m = re.search(r"Significant hits:\s*(.*?)(?:Details:|\Z)", txt_text, flags=re.S | re.I)
    if not m:
        return products
    ...
    for line in body.splitlines():
        ...
        if not line or not line[0].isdigit():
            continue
        # Format: 1. BGCxxxxxxx <product name>
        parts = re.split(r"\s+", line, maxsplit=2)
        if len(parts) >= 3:
            products.append(parts[2].strip())
...
def _parse_knownclusterblast(txt_path: Optional[Path], verbose: bool=False) -> Dict[str, str]:
    ...
    try:
        _kcb_txt = txt_path.read_text(encoding='utf-8', errors='replace')
    except Exception as _e:
        if verbose:
            print(f"[KCB] WARN: cannot read {txt_path}: {_e}")
        _kcb_txt = ''
    _all_sig_products = '; '.join(_kcb_list_significant_products(_kcb_txt))
    ...
    return {"BGC Accession": best["bgc"], **details, "KCB Products": _all_sig_products}
...
def parse_directory(
    ...
    fieldnames = [
        ...
        "TFBS Hits", "TFBS Types", "KCB Products"
    ]
